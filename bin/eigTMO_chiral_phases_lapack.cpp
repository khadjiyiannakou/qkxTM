...
#define ZERO ((qkxTMComplex) {0.,0.})
  ...
  qkxTMComplex phase_plus;
  qkxTMComplex phase_minus;
#define PI 3.14159265359
  phase_plus.real() = cos(PI/latInfo.L[3]);
  phase_plus.imag() = sin(PI/latInfo.L[3]);

  phase_minus.real() = cos(PI/latInfo.L[3]);
  phase_minus.imag() = -sin(PI/latInfo.L[3]);
  ...
  */
  ...
  for(int xp = 0 ; xp < latInfo.L[0]; xp++){
    ...
    A[MA(mu,a,iv,nu,b,ivp)] = -latInfo.kappa *( (DELTA(mu,nu) - g1[mu][nu])*u[0][MG(iv,a,b)]*DELTA(xp1,xp)*DELTA(y,yp)*DELTA(z,zp)*DELTA(t,tp)  + (DELTA(mu,nu) + g1[mu][nu])*conj(u[0][MG(ivmx,b,a)])*DELTA(xm1,xp)*DELTA(y,yp)*DELTA(z,zp)*DELTA(t,tp)  )
      ...
      -latInfo.kappa *( (DELTA(mu,nu) - g0[mu][nu])*phase_plus*u[3][MG(iv,a,b)] * DELTA(tp1,tp)*DELTA(x,xp)*DELTA(y,yp)*DELTA(z,zp)  + (DELTA(mu,nu) + g0[mu][nu])*phase_minus*conj(u[3][MG(ivmt,b,a)])*DELTA(tm1,tp)*DELTA(x,xp)*DELTA(y,yp)*DELTA(z,zp))
  ...
  FILE *ptr;
  ptr = fopen("EIG_chiral_phases.dat","w");
  for(int i = 0 ; i < NVEC ; i++)
    fprintf(ptr,"%+f\n",w[i].real());

  FILE *ptr2;
  ptr2 = fopen("EIGVECTORS_chiral_phases.dat","w");
  for(int i = 0 ; i < NVEC ; i++)
