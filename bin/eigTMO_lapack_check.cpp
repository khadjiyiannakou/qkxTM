...
extern "C" void zgemv_(char*,int*,int*,qkxTMComplex *,qkxTMComplex *,int*,qkxTMComplex*,int*,qkxTMComplex*,qkxTMComplex*,int*);
...
int main(){
  ...
  gaugefield->applyBoundary(&latInfo);


  int NL = arInfo.dimensionMatrix;  
  qkxTMComplex *A = (qkxTMComplex*)calloc(NL*NL,sizeof(qkxTMComplex));
  qkxTMComplex *V = (qkxTMComplex*)calloc(NL*NL,sizeof(qkxTMComplex));
  qkxTMComplex *V2 = (qkxTMComplex*)calloc(NL*NL,sizeof(qkxTMComplex));
...
#define ZERO ((qkxTMComplex) {0.,0.})
  ...
  // FILE *ptr;
  // ptr = fopen("EIG_quda.dat","w");
  // for(int i = 0 ; i < NVEC ; i++)
  //  fprintf(ptr,"%+f\n",w[i].real());

  FILE *ptr2;
  ptr2 = fopen("EIGVECTORS_chiral_phases.dat","r");
  for(int i = 0 ; i < NVEC ; i++)
    for(int iv = 0 ; iv < latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3]  ; iv++)
      for(int mu = 0 ; mu <4 ; mu++)
	for(int ic = 0 ; ic < 3 ; ic++)
	  fscanf(ptr2,"%lf %lf",&(V[i*4*3*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + mu*3*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + ic*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + iv].real()),&(V[ i*4*3*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + mu*3*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + ic*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + iv].imag()));

//SUBROUTINE ZGEMV ( TRANS, M, N, ALPHA, A, LDA, X, INCX, BETA, Y, INCY )

  qkxTMComplex transMatrix[4][4];

  for(int mu = 0 ; mu < 4 ; mu++)
    for(int nu = 0 ; nu < 4 ; nu++)
      transMatrix[mu][nu] = (qkxTMComplex) {0.,0.};
  double value = 1./sqrt(2.);

  transMatrix[0][0] = (qkxTMComplex) {-value,0.};
  transMatrix[1][1] = (qkxTMComplex) {-value,0.};
  transMatrix[2][2] = (qkxTMComplex) {+value,0.};
  transMatrix[3][3] = (qkxTMComplex) {+value,0.};

  transMatrix[0][2] = (qkxTMComplex) {+value,0.};
  transMatrix[1][3] = (qkxTMComplex) {+value,0.};
  transMatrix[2][0] = (qkxTMComplex) {+value,0.};
  transMatrix[3][1] = (qkxTMComplex) {+value,0.};
  ...
  for(int i = 0 ; i < NVEC ; i++)
    for(int it = 0 ; it < latInfo.L[3] ; it++)
      for(int iv3 = 0 ; iv3 < latInfo.L[0]*latInfo.L[1]*latInfo.L[2] ; iv3++)
	for(int ic = 0 ; ic < 3 ; ic++){
	  memset(tmp2,0,4*sizeof(qkxTMComplex));
	  
	  for(int mu = 0 ; mu < 4 ; mu++) tmp[mu] = V[i*4*3*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + mu*3*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + ic*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + it*latInfo.L[0]*latInfo.L[1]*latInfo.L[2] + iv3];
	  
	  for(int mu = 0 ; mu < 4 ; mu++)
	    for(int nu = 0 ; nu < 4 ; nu++)
	      tmp2[mu] = tmp2[mu] + transMatrix[mu][nu]*tmp[nu];
	  phase.real() = cos(PI*it/((double)latInfo.L[3]));
	  phase.imag() = sin(PI*it/((double)latInfo.L[3]));
	  //	  if(i == 0 && it == 0 && iv3 == 0 && ic == 0)printf("%e %e\n",phase.real(),phase.imag());
	  for(int mu = 0 ; mu < 4 ; mu++) V[i*4*3*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + mu*3*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + ic*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + it*latInfo.L[0]*latInfo.L[1]*latInfo.L[2] + iv3] = tmp2[mu] * phase;
  ...
  char Trans[] = "N";
  int inc=1;
  zgemv_(Trans,&nl,&nl,&alpha,AdagA,&nl,V,&inc,&beta,V2,&inc);


  //  printf("%+e %+e\n",V[0].real(),V[0].imag());

  double eigenValue = 3.065259;

  FILE *ptr3;
  ptr3 = fopen("EIGVECTORS_division.dat","w");
  for(int i = 0 ; i < 1 ; i++)
    for(int iv = 0 ; iv < latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3]  ; iv++)
      for(int mu = 0 ; mu <4 ; mu++)
	for(int ic = 0 ; ic < 3 ; ic++){
	  int pos = i*4*3*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + mu*3*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + ic*latInfo.L[0]*latInfo.L[1]*latInfo.L[2]*latInfo.L[3] + iv;
	  fprintf(ptr3,"%d %d %d %+e %+e \t %+e %+e\n",iv,mu,ic,V2[pos].real()/eigenValue,V2[pos].imag()/eigenValue,V[pos].real(),V[pos].imag());
	}
