...
int process_command_line_option(int argc, char** argv, int* idx)
{
  ...
  if( strcmp(argv[i], "--mu") == 0){
    ...
    printf("Overriding mu with %le for strange/charm computation.\n", newMu);
  ...
  if( strcmp(argv[i], "--load") == 0){
	...
	nConf = atoi(argv[i+1]); 
	//    	sprintf(latfile,"/data/avaquero/confs/D15.48/conf.%04d", nConf);
    	sprintf(latfile,"/home/khadjiyiannakou/confs/conf.%04d", nConf);
	printf	("Will load %s\n", latfile);
    i++;
    ret = 0;
    goto out;
