  ...
  : nspins(NSPINS) , ncolors(NCOLORS), ndf(NDF),evenOdd_order(false)
{
  ...
  totalBytes = volume*nspins*ncolors*sizeof(qkxTMComplex);
...
{
  totalBytes = volume*ncolors*ncolors*NDIM*sizeof(qkxTMComplex);
}

void GaugeField::create()
{
  qkxTMComplex** pointer; // number of direction are 4
  
  pointer = (qkxTMComplex**)malloc(NDIM*sizeof(qkxTMComplex*));
  ...
  for( int i = 0 ; i < NDIM ; i++){
    pointer[i] = (qkxTMComplex*)malloc(volume*ncolors*ncolors*sizeof(qkxTMComplex));
    if( pointer[i] == NULL ){
      fprintf(stderr,"Error with memory allocation of gauge field\n");
      exit(EXIT_FAILURE);
    }
...
void GaugeField::copy(GaugeField &src){
  ...
  src_p = src.p_gauge;
    ...
    for(int i = 0 ; i < NDIM ; i++)
      memcpy(&(dst_p[i][0]),&(src_p[i][0]),volume*ncolors*ncolors*sizeof(qkxTMComplex));
...
void GaugeField::zero(){
  ...
  for(int i = 0 ; i < NDIM ; i++)
    memset( &(this->p_gauge[i][0]) , 0 ,volume*ncolors*ncolors*sizeof(qkxTMComplex));
...
void GaugeField::applyBoundary(LatticeInfo *latInfo){
  ...
  qkxTMComplex **gauge = this->p_gauge;
...
double GaugeField::calculatePlaquette()
{
  ...
  for(int iv = 0 ; iv < volume ; iv++){     // summation over the volume
	...
	memcpy(A->M,&(p_gauge[mu][MG(iv,0,0)]),9*sizeof(qkxTMComplex));
	memcpy(B->M,&(p_gauge[nu][MG(fwd[mu],0,0)]),9*sizeof(qkxTMComplex));
	memcpy(C_dag->M,&(p_gauge[mu][MG(fwd[nu],0,0)]),9*sizeof(qkxTMComplex));
	memcpy(D_dag->M,&(p_gauge[nu][MG(iv,0,0)]),9*sizeof(qkxTMComplex));
	C_dag->dagger();
	D_dag->dagger();
	*E = (*A)*(*B)*(*C_dag)*(*D_dag);
	temp += E->traceColor();
...
void GaugeField::APEsmearing(qkxTMComplex **U_tmp, qkxTMComplex **U_ape, LatticeInfo *latInfo){
  ...
  qkxTMComplex **in = NULL;
  qkxTMComplex **out = NULL;
  qkxTMComplex **tmp = NULL;
  ...
  for(int iter = 0 ; iter < nsmear ; iter++){
	...
	for(int nu = 0 ; nu < 3 ; nu++){
	  if(mu != nu){
	    ...
	    // forward staple
	    memcpy(A->M,&(in[nu][MG(iv,0,0)]),9*sizeof(qkxTMComplex));
	    memcpy(B->M,&(in[mu][MG(fwd[nu],0,0)]),9*sizeof(qkxTMComplex));
	    memcpy(C_dag->M,&(in[nu][MG(fwd[mu],0,0)]),9*sizeof(qkxTMComplex));
	    C_dag->dagger();
	    *D = *D + (*A)*(*B)*(*C_dag);                          // U_\nu(x) * U_\mu(x+\nu) * ( U_\nu(x+\mu) )^dagger


	    // backward stable
	    memcpy(A_dag->M,&(in[nu][MG(bwd[nu],0,0)]),9*sizeof(qkxTMComplex));
	    memcpy(B->M,&(in[mu][MG(bwd[nu],0,0)]),9*sizeof(qkxTMComplex));
	    memcpy(C->M,&(in[nu][MG(fwdBwd[mu][nu],0,0)]),9*sizeof(qkxTMComplex));
	    A_dag->dagger();
	    *D = *D + (*A_dag)*(*B)*(*C);                               // ( U_\nu(x-\nu))^dagger * U_\mu(x-\nu) * U_\nu(x+\mu-\nu)
	...
	memcpy(E->M,&(in[mu][MG(iv,0,0)]),9*sizeof(qkxTMComplex));
	*F = *E + (*D)*alpha;
	memcpy(&(out[mu][MG(iv,0,0)]) ,F->M , 9*sizeof(qkxTMComplex) );
      } // for mu
    } // close volume
    ...
    if( (nsmear%2) == 0){
      for(int i = 0 ; i < 3 ; i++)
	memcpy(U_ape[i],in[i],volume*ncolors*ncolors*sizeof(qkxTMComplex));
    }
  
    memcpy(U_ape[3],U_tmp[3],volume*ncolors*ncolors*sizeof(qkxTMComplex));
...
void ColorSpinorField::create()
{
  ...
  pointer = (qkxTMComplex**)malloc(nspins*ncolors*sizeof(qkxTMComplex*));
  ...
  for( int i = 0 ; i < nspins*ncolors ; i++){
    pointer[i] = (qkxTMComplex*)malloc(volume*sizeof(qkxTMComplex));
    if( pointer[i] == NULL ){
      fprintf(stderr,"Error with memory allocation of gauge field\n");
      exit(EXIT_FAILURE);
    }
...
void ColorSpinorField::copy(ColorSpinorField &src){
  ...
  for(int mu = 0 ; mu < nspins ; mu++)
    for(int ic = 0 ; ic < ncolors ; ic++)
      memcpy(&(dst_p[MS(mu,ic)][0]),&(src_p[MS(mu,ic)][0]) , volume*sizeof(qkxTMComplex) );
...
void ColorSpinorField::zero(){
  ...
  for(int mu = 0 ; mu < nspins ; mu++)
    for(int ic = 0 ; ic < ncolors ; ic++)
      memset(&(this->p_colorSpinor[MS(mu,ic)][0]), 0 , volume*sizeof(qkxTMComplex) );
...
void ColorSpinorField::gaussianSmearing(qkxTMComplex **Psi_tmp, qkxTMComplex **Psi_sm, qkxTMComplex **gauge , LatticeInfo *latInfo)
{
  ...
  for(int iter = 0 ; iter < nsmear ; iter++){
	...
	memcpy(U->M, &(gauge[mu][MG(iv,0,0)]), 9*sizeof(qkxTMComplex));
	memcpy(U_back->M, &(gauge[mu][MG(bwd[mu],0,0)]), 9*sizeof(qkxTMComplex));
	U_back->dagger();
  ...
  if( (nsmear%2) == 0){
  for( int i = 0 ; i < nspins*ncolors ; i++)
    memcpy(Psi_sm[i],in[i], volume*sizeof(qkxTMComplex) );
  }
...
void ColorSpinorField::applyDslash(qkxTMComplex **Psi, qkxTMComplex **gauge){
...
void ColorSpinorField::applyTwistAddDslash(qkxTMComplex **Dslash, qkxTMComplex **Psi, LatticeInfo *latInfo){

  spinColor *phi = new spinColor();
  qkxTMComplex twistCoeff;
...
void ColorSpinorField::applyDslashDag(qkxTMComplex **Psi, qkxTMComplex **gauge){
...
void ColorSpinorField::applyTwistDagAddDslashDag(qkxTMComplex **DslashDag, qkxTMComplex **Psi, LatticeInfo *latInfo){
...
void ColorSpinorField::MdagM(qkxTMComplex **vec,qkxTMComplex **gauge, LatticeInfo *param){
